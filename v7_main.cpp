...
public:
  ...
  bool test(int bit)
  {
    return presence[bit/BITMAP_SIZE].test(bit % BITMAP_SIZE);
  }
  ...
  void set(int bit)
  {
    ...
    if(parent != nullptr)//TODO: remove the branch
    {
      ...
      if(!presence[presenceNumber].test(offset))
      {
        presence[presenceNumber].set(offset, true);
        parent->set(presenceNumber);
      }
    }
    else
    {
      presence[0].set(bit, true);
    }
  }
  ...
  void unset(int bit)
  {
    ...
    if(parent != nullptr)//TODO: remove the branch
    {
      ...
      if(presence[presenceNumber].test(offset))
      {
        presence[presenceNumber].set(offset, false);
        parent->unset(presenceNumber);
      }

    }
    else
    {
      presence[0].set(bit, false);
    }
  }
...
public:
  ...
  void addOrder(int32_t N, ResultAtT* result)//NO return to force RVO
  {
    ...
    orders[currentT].N = N;
    //if(entries[entryNumber].addExpiringOrder( &orders[currentT]))
    //{
    ...
    int nextEntryToExpire = minHeap.firstAfter(orderNumber - 1);

    nextExpiringEntry = &entries[nextEntryToExpire];
    result->nextToExpire = nextExpiringEntry->expiringHead->id;
...
/*
    ...
    */
    ...
    std::cin >> N;
    int maxN = N;
    int *ns = new int[128000000];
    int nNs = 0;
    char buffer[1000000];
    int bufferSize = 0;
    int maxBuffSize = 1000000 - 100;
...
high_resolution_clock::time_point t1 = high_resolution_clock::now();
    for(int i = 0 ; i < nNs; i++)
    {
      ob->addOrder(ns[i], &result);


      bufferSize+= sprintf(buffer + bufferSize, "%d %d %d\n%d\n", result.nextToExpire,result.maxN, result.noOfLiveOrders, result.noOfOrdersExpiringAtT );
